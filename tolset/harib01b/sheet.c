...
void sheet_updown(struct SHEET *sht, int height)
{
	...
	if (old > height) { // lower than before
		...
		sheet_refreshsub(ctl, sht->vx0, sht->vy0, sht->vx0 + sht->bxsize, sht->vy0 + sht->bysize);   // repaint the frame according the new sheets
	} else if (old < height) {
		if (old >= 0) {
			//move down
			for (h = old; h < height; h++) {
...
void sheet_slide(struct SHEET *sht, int vx0, int vy0)
{
	...
	sht->vy0 = vy0;
	if (sht->height >= 0) {
		sheet_refreshsub(sht->ctl, old_vx0, old_vy0, old_vx0 + sht->bxsize, old_vy0 + sht->bysize); // remove the old traces
		sheet_refreshsub(sht->ctl, vx0, vy0, vx0 + sht->bxsize, vy0 + sht->bysize);                 // draw the new traces
	}
	return;
}
